"""In-memory outbound adapters for deterministic tests."""
...
class InMemoryKnowledgeBaseRepository:
    def __init__(self) -> None:
        self._items: dict[tuple[TenantId, KnowledgeBaseId], KnowledgeBase] = {}

    def add(self, knowledge_base: KnowledgeBase) -> None:
        key = (knowledge_base.tenant_id, knowledge_base.id)
        if key in self._items:
            raise ValueError("knowledge base already exists")
        self._items[key] = knowledge_base

    def get(
        self, tenant_id: TenantId, knowledge_base_id: KnowledgeBaseId
    ) -> KnowledgeBase | None:
        return self._items.get((tenant_id, knowledge_base_id))


class InMemoryTransaction(AbstractContextManager[None]):
    def __init__(self) -> None:
...
class InMemoryObjectStore:
    def __init__(self) -> None:
        self._objects: dict[tuple[TenantId, str], bytes] = {}
...
class InMemoryMessageQueue:
    ...
    def publish(self, topic: str, message_id: str) -> None:
        self.messages.append((topic, message_id))


class HealthySearchIndex:
